...
def main():
    ...
    state = []
    with io.open("alacritty.install.ghstate", "a+") as f:
        f.seek(0)  # why I have to do this is unknown
        for _, line in enumerate(f):
            state.append(line)
    f = io.open("alacritty.install.ghstate", "a+")
    ...
    for rel in alacritty.get_releases():
        ...
        if not pushed:
            ...
            subprocess.call(["wget",
                             ...
                             "--output-document",
                             "alacritty.msi"])
            chksum = checksum.get_for_file("alacritty.msi", "sha512")
            os.remove("alacritty.msi")
            ...
            abort_on_nonzero(subprocess.call(["choco",
                                              "pack",
                                              Path(tempdir)/"alacritty.install.nuspec"]))
            f.write(str(rel.id)+"\n")
            f.flush()
        else:
            continue
...
def get_correct_release_asset(assets):
    for a in assets:
        if "windows-installer" in a.name:
...
def find_and_replace_templates(directory,
    ...
    d = dict(version=version,
             ...
             filename=filename,
             notes=notes)
    basepath = Path(os.getcwd())/"src/templates/alacritty.install/"
    templates = [
            "alacritty.install.nuspec",
