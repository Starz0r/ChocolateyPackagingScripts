...
def main():
    logger = logging.getLogger('Alacritty Install GitHub Releases')
    logger.setLevel(logging.DEBUG)

    for rel in on_new_git_release("alacritty.install", "alacritty/alacritty"):
        # correlate assets
        asset = get_correct_release_asset(rel.get_assets(), ".msi",
                                          None)
        ...
        find_and_replace_templates_new("alacritty.install", tmpdir, d)
        os.rename(fname, os.path.join(tmpdir, "tools", fname))
        abort_on_nonzero(
            subprocess.call(["choco", "pack",
                             Path(tmpdir) / "alacritty.install.nuspec"]))
