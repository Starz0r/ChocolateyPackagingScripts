...
def main():
    ...
    for tag in on_each_git_tag("neochat.install", "KDE/neochat"):
        ...
        d = dict(version=version, tag=gittag, checksum=chksum, fname=fname, url=dl_url, notes=relnotes)
        ...
        abort_on_nonzero(subprocess.call(["choco", "pack", Path(tmpdir) / "neochat.install.nuspec"]))
