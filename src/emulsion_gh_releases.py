...
def main():
    logger = logging.getLogger('Emulsion GitHub Releases')
    logger.setLevel(logging.DEBUG)

    for rel in on_new_git_release("emulsion", "arturkovacs/emulsion"):
        # correlate assets
        ...
        find_and_replace_templates_new("emulsion", tmpdir, d)
        os.rename(fname, os.path.join(tmpdir, "tools", fname))
        abort_on_nonzero(
            subprocess.call(["choco", "pack",
                             Path(tmpdir) / "emulsion.nuspec"]))
